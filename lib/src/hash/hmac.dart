...
class HMAC {
  ...
  Uint8List get digest {
    ...
    List<int> innerHash = hash.convert(iKeyPad + message).bytesList;
    List<int> finalHash = hash.convert(oKeyPad + innerHash).bytesList;
  ...
  Uint8List _normalizeKey(List<int> key) {
    ...
    if (normalizedKey.length > digestSize) {
      normalizedKey = Uint8List.fromList(hash.convert(key).bytesList);
    }
    if (normalizedKey.length < digestSize) {
